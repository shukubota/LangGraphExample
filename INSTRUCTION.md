# INSTRUCTION.md - ClaudeCode実装指示書
...
```
paper-analyzer/
├── .envrc                    # direnv設定（ANTHROPIC_API_KEY）
├── paper_analyzer.py         # メインファイル
...
1. **環境変数設定**
   ```bash
   echo 'export ANTHROPIC_API_KEY="your-key"' > .envrc
   direnv allow
   ```
...
3. **実装**
   - 上記のコードを`paper_analyzer.py`にまとめる
   - 各関数は独立しているので順次実装可能

4. **テスト実行**
   ```bash
   python paper_analyzer.py test_paper.pdf
   ```

## ⚠️ 注意点

1. **APIキー管理**: 必ずdirenvで管理し、.envrcは.gitignoreに追加
2. **PDF処理**: 長い論文は最初の15ページのみ処理（調整可能）
3. **エラーハンドリング**: APIエラー、PDF読み込みエラーに対応
4. **テキスト切り詰め**: プロンプトが長すぎる場合は適宜制限

## 🎯 期待される動作

1. PDFを読み込み
2. 5つのAgentが順次/条件付きで実行
3. 処理ログを表示
4. 「5分で分かる論文解説」をMarkdown形式で生成
5. ファイルに保存
